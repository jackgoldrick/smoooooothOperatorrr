...
if nargin == 8
    guessDim1 = size(diagonalStackBGuess, 1);
    guessDim3 = size(diagonalStackBGuess, 3);
    if [guessDim1, guessDim3] == [dim1, dim2]
        diagonalStackB = cat(4, diagonalStackB, diagonalStackBGuess);
        sMaxDiag = size(diagonalStackB, 4);
        clear dimsTemp;
    else
        fprintf('\nB Diagonal guess not correct data type\n')
    end
end
