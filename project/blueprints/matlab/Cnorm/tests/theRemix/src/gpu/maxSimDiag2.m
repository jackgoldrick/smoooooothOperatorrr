...
dim1 = dims(2);
dim2 = dims(1);
%diagonalStackA = gpuArray(diagonalStackA);
...
[pNormStackA, ~] = pPower2(stackA, p, err_a, sMaxPower);
...
while (true)
    ...
    [normStackB(:,:,~flatConverged), vMax(:,:,~flatConverged)] = pPower2(stackB(:,:,~flatConverged), p, err_a, sMaxPower, vMax(:,:,~flatConverged));
