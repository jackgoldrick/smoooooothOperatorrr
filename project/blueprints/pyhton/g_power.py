...
def p_power(matrix, p, err_a=1e-6, s_max=25, v_init=None, max_breaders=4, lyfe_cycle=0):
    '''
    Matrix operator p-norm along last two dimensions of matrix.
    '''
    generation  = 1
    max_guesses = tc.zeros(lyfe_cycle+1, 1)
    ...
    while(True):
        ...
        max_guesses[generation-1] = guesses[ind[0], 0, 0, 0]
        
        if tc.max(abs(max_guesses[generation-1] - old_guess)) < err_a:
            ...
            v_max = x[mask].reshape(dims[0], dims[3], max_breaders)
            # max_guess = guess.squeeze(1)
