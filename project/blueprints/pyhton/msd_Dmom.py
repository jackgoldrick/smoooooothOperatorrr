...
def max_sim_diag(
    ...
    s_max_power=100, err_a=1e-6, err_scale=100, momentum=1e-5
):
    ...
    t = 1
    mom_vector = tc.zeros(s_max_diag, 1, 1, 1).to(device)
    while True:
        ...
        converged |= (abs(guess - norm_stack_A) <= err_a) | (guess - old_guess <= err_a * 1)
        ...
        step[converged] = 0
        diags_B += step * gradient - momentum * mom_vector
        mom_vector = gradient 
        t += 1
        old_guess = guess
